...
GradientMap.prototype.getImage = function () {
    var image = this.canvas.toDataURL("image/png");
    return image;
};

GradientMap.prototype.getCleanImage = function (level) {
    if(level === undefined){ level = 250};
    var imageData = this.ctx.getImageData(0,0, this.size.width, this.size.height),
    data = imageData.data;
    ...
    var image = this.canvas.toDataURL("image/png");
    // downloadBase64File(image, 'rain.png')
    return image;
};

GradientMap.crossProduct = function (o, a, b) {
    return (a.x - o.x) * (b.y - o.y) - (a.y - o.y) * (b.x - o.x);
};

GradientMap.pointInPolygon = function (point, vs) {
    var x = point.x,
        y = point.y,
        inside = false,
        i = 0,
...
GradientMap.squareDistance = function (p0, p1) {
    var x = p0.x - p1.x,
        y = p0.y - p1.y;
    return x * x + y * y;
};

GradientMap.prototype.getColor = function (levels, value) {
    var max = 30,
        min = 2,
        val = value/max
        if (val < min/max) {
    ...
    return d3.interpolateBlues(val).match(/\d+/g).map(Number);
};
var pointsGradient = []
GradientMap.prototype.getPointValue = function (limit, point) {
    pointsGradient.push(point);
    var counter = 0,
        arr = [],
        tmp = 0.0,
        dis = 0.0,
    ...
    if (GradientMap.pointInPolygon(point, this.polygon)) {
        for (counter = 0; counter < this.points.length; counter = counter + 1) {
            dis = GradientMap.squareDistance(point, this.points[counter]);
            if (dis === 0) {
...
GradientMap.prototype.setConvexhullPolygon = function (points) {
    var lower = [],
        upper = [],
        i = 0;
    ...
    for (i = 0; i < points.length; i = i + 1) {
        while (lower.length >= 2 && GradientMap.crossProduct(lower[lower.length - 2], lower[lower.length - 1], points[i]) <= 0) {
            lower.pop();
        };
        lower.push(points[i]);
    }
    ...
    for (i = points.length - 1; i >= 0; i = i - 1) {
        while (upper.length >= 2 && GradientMap.crossProduct(upper[upper.length - 2], upper[upper.length - 1], points[i]) <= 0) {
            upper.pop();
        };
        upper.push(points[i]);
    };
...
GradientMap.prototype.setPoints = function (arr, width, height) {
    this.points = arr;
    this.width = width;
    this.height = height;
...
GradientMap.prototype.setRandomPoints = function (points, width, height) {
    var counter = 0,
        x = 0,
        y = 0,
        v = 0.0,
...
GradientMap.prototype.drawLow = function (limit, res, clean, cutoff, callback) {
    var self = this,
        ctx = this.ctx,
        dbl = 2 * res,
        col = [],
    ...
    for (x = xBeg; x < xEnd; x = x + res) {
        for (y = yBeg; y < yEnd; y = y + res) {
            ...
            if (val !== -255) {
                ...
                //// remove 'white' pixels setting alpha to 0
                if(col[0] >= cutoff&& col[1] >= cutoff && col[2] >= cutoff){
                    var alpha = '0)'
                } else {
                    var alpha = '0.5)'
                }
                gradient.addColorStop(0, str + alpha);
...
GradientMap.prototype.drawFull = function (levels, callback) {
    'use strict';
    var self = this,
        ctx = this.ctx,
...
GradientMap.prototype.drawPoints = function (callback) {
    var self = this,
        PI2 = 2 * Math.PI,
        ctx = this.ctx;
    window.requestAnimationFrame(function (timestamp) {
...
function downloadBase64File(imageBase64, fileName) {
