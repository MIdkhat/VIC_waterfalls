...
for ($i = 0; $i < count($stations_combined); $i++) {
    ...
    //// GET THE RAIN AMOUNT ON THE DATE
    if($raw_data[$day-1][$month] == ""){
        $val = '0';
    } else {
        $val = strval(round($raw_data[$day-1][$month], 1));  //// get yesterdays rain in mm
    }

    array_push($stations_combined[$i]['rain'], array('date' => $date, 'value' => $val));

    if (count($stations_combined[$i]['rain']) > $days_back){
        array_shift($stations_combined[$i]['rain']);
    }
